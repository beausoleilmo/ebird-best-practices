...
Next, we use a model selection approach to compare and rank our candidate model set. For this example, we use the `dredge()` function to explore a candidate set consisting of all possible additive combinations of the gobal model. However, we suggest that a more thoughtful approach be considered for specific applications and different types of predictor variables.
