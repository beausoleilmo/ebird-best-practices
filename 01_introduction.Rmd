...
The first two chapters of this book focus on obtaining and preparing eBird and habitat data for the modeling that will occur in the remaining chapters. These steps can be time consuming and laborious. If you'd like to skip straight to the analysis, [**download this package of prepared data**](https://github.com/mstrimas/ebird-best-practices/raw/master/data/data.zip). Unzip this file so that the contents are in the `data/` subdirectory of your RStudio project folder. This will allow you to jump right in to the modeling and ensure that you're using exactly the same data as was used when creating this book.
...
library(auk)
# set ebd path
auk::auk_set_ebd_path("data/ebd_ecuador/ebd_EC-W_relNov-2019/", overwrite = TRUE)
auk_ebd("data/ebd_ecuador/ebd_EC-W_relNov-2019/ebd_EC-W_relNov-2019.txt")
# auk_get_ebd_path()
```

After restarting your R session, you should be able to refer directly to the EBD or Sampling Event Data files (e.g., `auk_ebd("ebd_relAug-2018.txt")`). Provided your collaborators have also set `EDB_PATH`, your scripts should now be portable.
...
gpkg_dir <- "data/geo_data"
if (!dir.exists(gpkg_dir)) {
  dir.create(gpkg_dir)
}
f_ne <- file.path(gpkg_dir, "gis-data_EC.gpkg")
...
ne_country_lines <- st_intersects(ne_country_lines, ne_land, sparse = FALSE) %>%
  as.logical() %>%
  {ne_country_lines[.]}
# states, South america # see http://www.virtualherbarium.org/vh/auth/country.html 
# ne_state_lines <- ne_download(scale = 50, category = "cultural",
#                               type = "admin_1_states_provinces_lines",
#                               returnclass = "sf") %>%
#   filter(adm0_a3 %in% c("ECU")) %>%
#   mutate(iso_a2 = recode(adm0_a3, USA = "US", CAN = "CAN")) %>% 
#   select(country = adm0_name, country_code = iso_a2)

# output
galapagos_sf <- geojsonsf::geojson_sf("data/remote_sensing/gal.json")
galapagos_sf_union = st_union(galapagos_sf)
unlink(f_ne)

write_sf(galapagos_sf_union, f_ne, "galapagos_islands")
...
While using the GitHub repository, the local storage of the data in the tutorial might increase. You can clean the local repository with this command
```{bash}
cd ~/Dropbox/Git/ebird-best-practices/
