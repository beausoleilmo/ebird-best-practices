...
- **Spatial precision:** the spatial location of an eBird checklist is given as a single latitude-longitude point; however, this may not be precise for two main reasons. First, for traveling checklists, this location represents just one point on the journey. Second, eBird checklists at often assigned to a *hotspot* (a common location for all birders visiting a popular birding site) rather than their true location. For these reasons, it's not appropriate to align the eBird locations with very precise habitat covariates, rather we recommend summarizing covariates within a neighborhood around the checklist location.
- **Class imbalance:** bird species that are rare or hard to detect may have data with high class imbalance, with many more checklists with non-detections than detections. For these species, a distribution model predicting the species is absent everywhere will have high accuracy, but no ecological value. We'll follow the methods for addressing class imbalance proposed by Robinson et al. @robinsonUsingCitizenScience2018.
- **Observer variation:** eBird data are collected from participants with a wide variety of behavior, experience, and skill in detecting and identifying species [@kellingCanObservationSkills2015]. To address this, each checklist is assigned a *Checklist Calibration Index (CCI)*, which calibrates observers and checklists against others from similar times and places and accounts for variation in observer behavior, equipment, and skill at detecting species. [@johnstonEstimatesObserverExpertise2018; @kellingCanObservationSkills2015]
...
Due to the massive size of the eBird dataset, working with it requires the Unix command-line utility AWK. You will not need to use AWK directly, as the R package auk does this hard work for you, but you do need AWK to be installed on your computer. Linux and Mac users should already have AWK installed on their machines; however, Windows user will need to [install Cygwin](https://www.cygwin.com/) to gain access to AWK. Cygwin is free software that allows Windows users to use Unix tools. Note that Cygwin should be installed in the default location (`C:/cygwin/bin/gawk.exe` or `C:/cygwin64/bin/gawk.exe`) in order for everything to work correctly. 
...
remotes::install_github("CornellLabofOrnithology/ebird-best-practices")
```

Note that several of the spatial packages require dependencies. If installing these packages fails, consult the [instructions for installing dependencies on the `sf` package website](https://r-spatial.github.io/sf/#installing). Finally, **ensure all R packages are updated** to the most recent by clicking on the Update bottom on the Packages tab in RStudio.

### Tidyverse {#intro-setup-tidyverse}

Throughout this website, we use packages from the [Tidyverse](https://www.tidyverse.org/), an opinionated collection of R packages designed for data science. Packages such as [`ggplot2`](http://ggplot2.tidyverse.org/), for data visualization, and [`dplyr`](http://dplyr.tidyverse.org/), for data manipulation, are two of the most well known Tidyverse packages; however, there are many more. Throughout this website, we often use Tidyverse functions without explanation. If you encounter a function you're unfamiliar with, consult the documentation for help (e.g. `?mutate` to see help for the `dplyr` function `mutate`). More generally, the free online book [R for Data Science](http://r4ds.had.co.nz/) by [Hadley Wickham](http://hadley.nz/) is the best introduction to working with data in R using the Tidyverse.
...
Throughout this book, we'll be producing maps of species distributions. To provide context for these distrbutions, we'll need GIS data on on political boundaries. [Natural Earth](https://www.naturalearthdata.com/) is the best source for a range of tightly inegrated vector and raster GIS data for producing professional cartographic maps. The R package, [`rnaturalearth`](https://github.com/ropensci/rnaturalearth) provides a convenient method for accessing these data from R. We'll also need [Bird Convservation Region (BCR)](http://nabci-us.org/resources/bird-conservation-regions/) boundaries, which are available through [Bird Studies Canada](https://www.birdscanada.org/research/gislab/index.jsp?targetpg=bcr&targetpg=bcr).
